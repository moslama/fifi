<?php
	...
	// Is Admin
	function is_admin() {
		if ($_SESSION['uuid'] && $_SESSION['uuid'] !== "" && $_SESSION['ranki'] == 1)
			return (true);
		return (false);
	}
	...
	function modif($post, $connect) {
		if (!empty($post['mail']) && !empty($post['password1'])) {
			if (preg_match('/@.+\./', $post['mail'])) {
				$sql = "SELECT uuid, mail, password, create_at, ranki FROM users WHERE uuid='".htmlspecialchars($_SESSION['uuid'])."'";
				$result = mysqli_query($connect, $sql);
				if (mysqli_num_rows($result) > 0) {
					$user = mysqli_fetch_assoc($result);
					if ($user['password'] === pass_hash($user['mail'], $post['password1'])) {
						$sql = "UPDATE users SET mail='".$post['mail']."', password='".pass_hash($post['mail'], $post['password1'])."'  WHERE uuid='".htmlspecialchars($_SESSION['uuid'])."'";
	...
	function signup($post, $connect) {
		if (!empty($post['mail']) && !empty($post['password1']) && !empty($post['password1'])) {
			if (preg_match('/@.+\./', $post['mail'])) {
				$sql = "SELECT uuid, mail, password, create_at, ranki FROM users WHERE mail='".htmlspecialchars($post['mail'])."'";
				$result = mysqli_query($connect, $sql);
				if (mysqli_num_rows($result) == 0) {
